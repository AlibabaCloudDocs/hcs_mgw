...
|SourceAddrBadHttpListFormat|HTTP/HTTPS列表文件格式错误。列分隔符为'\\t'，行分隔符为'\\n'。|请修改列表地址中的列表文件格式后重试。列表文件格式请参见[创建列表文件](../../../../intl.zh-CN/HTTP HTTPS 源迁移教程/准备工作.md#ol_gtr_vzd_qfb)。|
...
|SourceAddrEndpointOrSkInvalid|KS3源数据地址Endpoint无效或AccessKeySecret不正确。|请填写正确的源地址Endpoint或AccessKeySecret。Endpoint详情请参见[附录：地域与域名对应关系表](../../../../intl.zh-CN/金山云 KS3 迁移教程/迁移实施.md#region)。|
